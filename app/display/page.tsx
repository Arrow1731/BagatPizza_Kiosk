    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      {newReadyOrder && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50">
          <div className="bg-green-500 px-6 py-4 rounded-xl shadow-xl flex items-center gap-4">
            <Bell className="h-6 w-6" />
            <span className="text-xl font-bold">Buyurtma #{newReadyOrder} tayyor!</span>
      ...
      <header className="text-center py-6 border-b border-white/10">
        ...
        {currentTime && (
          <div className="mt-2 text-purple-300 flex items-center justify-center gap-2">
      ...
      <div className="flex h-[calc(100vh-150px)]">
        ...
        <div className="w-1/2 p-6 border-r border-white/10 overflow-y-auto">
          <h2 className="text-3xl text-orange-400 font-bold mb-4">Tayyorlanmoqda</h2>
          {sortedPreparingOrders.length === 0 ? (
            <p className="text-orange-300">Buyurtmalar yo'q</p>
          ) : (
            sortedPreparingOrders.map((order) => (
              <div key={order.id} className="bg-orange-400/20 p-4 rounded-xl mb-4">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-xl font-bold">Buyurtma #{order.orderNumber}</div>
                    <div className="text-sm text-orange-200">Jarayonda</div>
                  </div>
        ...
        <div className="w-1/2 p-6 overflow-y-auto">
          <h2 className="text-3xl text-green-400 font-bold mb-4">Tayyor</h2>
          {sortedReadyOrders.length === 0 ? (
            <p className="text-green-300">Hozircha tayyor buyurtma yo'q</p>
          ) : (
            sortedReadyOrders.map((order) => {
              const timeInfo = getRemainingTime(order.timestamp)
              return (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <Badge className="bg-green-500 text-white">{timeInfo.time}</Badge>
                    </div>
                  </div>
                </div>
              )
      ...
      {/* <footer className="text-center py-4 text-sm text-purple-200 border-t border-white/10">
        Rahmat! O'rtacha kutish vaqti: 10-15 daqiqa
      </footer> */}
    </div>
  )
}