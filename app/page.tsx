...
export default function KioskMenu() {
  ...
  const categories = ["Hammasi", "Pizza", "Burger", "Drink", "Fries", "Water", "Chicken"]
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-red-50 to-pink-50">
      ...
      <header className="bg-gradient-to-r from-red-600 via-red-700 to-orange-600 text-white shadow-2xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold text-white">Bog'ot Pizza</h1>
              <p className="opacity-90">O'z-o'ziga xizmat ko'rsatish kioskasi</p>
            </div>
            <Link href="/cart">
              <Button className="bg-white text-red-600 shadow hover:shadow-md">
                <ShoppingCart className="mr-2" />
                Savat ({totalItems})
                <Badge className="ml-2 bg-red-600 text-white">{totalPrice} so'm</Badge>
              </Button>
            </Link>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {filteredItems.length === 0 ? (
          <p className="text-center text-gray-600">Bu kategoriyada mahsulotlar yo‘q.</p>
        ) : (
            ...
            {filteredItems.map((item) => (
              <Card key={item.id} className="overflow-hidden hover:shadow-lg transition">
                <div className="aspect-video relative">
                  ...
                  />
                  {item.popular && (
                    <Badge className="absolute top-2 left-2 bg-red-600 text-white">Xit</Badge>
                  )}
                </div>
                <CardHeader>
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <div className="text-lg font-bold text-red-600">{item.price} so'm</div>
                    {item.rating && (
