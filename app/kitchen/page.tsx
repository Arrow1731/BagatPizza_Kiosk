...
export default function KitchenDashboard() {
  ...
  // Функция для получения эмодзи способа доставки
  const getDeliveryMethodEmoji = (method: string) => {
    switch (method) {
      case "delivery":
        return "🚚"
      case "takeaway":
        ...
        <div className="mb-8">
          ...
          {preparingOrders.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {preparingOrders
                .sort((a, b) => {
                  const numA = Number.parseInt(a.orderNumber) || 0
                  const numB = Number.parseInt(b.orderNumber) || 0
                  return numA - numB
                })
                .map((order, index) => {
                  const isPrinting = printingOrders.has(order.id || "")
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {readyOrders
                ...
                .map((order, index) => (
                  <Card key={order.id || `ready-${index}`} className="border-green-300 bg-green-50 animate-pulse">
                    <CardContent className="p-4 text-center">
                      <div className="text-3xl font-bold text-green-600 mb-2">#{order.orderNumber}</div>
                      <div className="flex justify-center gap-1 mb-2">
                        <Badge variant="outline" className="text-xs bg-green-50 text-green-700 border-green-200">
                          {getDeliveryMethodEmoji(order.deliveryMethod)}
                        </Badge>
                        <Badge variant="outline" className="text-xs bg-orange-50 text-orange-700 border-orange-200">
                          {order.paymentMethod === "card" ? "💳" : "💵"}
                        </Badge>
