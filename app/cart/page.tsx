...
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { ArrowLeft, Plus, Minus, Trash2, CreditCard, Banknote, Loader2, Truck, ShoppingBag, Coffee } from "lucide-react"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { useFirebaseCart } from "@/hooks/useFirebaseCart"
import { useFirebaseOrders } from "@/hooks/useFirebaseOrders"
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold">Sizning buyurtmangiz</h1>
              <p className="text-lg opacity-90">
                {totalItems} mahsulot narxi {totalPrice.toLocaleString()} UZS
              </p>
            </div>
          </div>
        </div>
                ...
                <CardContent className="p-4 sm:p-6">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-lg sm:text-xl">{item.name}</h3>
                      <p className="text-gray-600 text-sm sm:text-base line-clamp-2">{item.description}</p>
                      <p className="text-red-600 font-bold text-xl sm:text-2xl mt-2">
            ...
            <Card className="shadow-lg">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-2xl font-bold">
                    <span>Umumiy</span>
                    <span className="text-red-600">{totalPrice.toLocaleString()} UZS</span>
                  </div>
                </div>
                <Button
                  size="lg"
